...
export default function WelcomeTemplateWithProvider(userDetails: userDetails) {
    ...
    <Html>
      ...
      <Tailwind>
        <Container className="bg-white px-2 mt-4 w-full text-sm">
          <Text className="text-lg text-black">Dear {userDetails.name},</Text>
          <Text className="text-gray-700">Welcome to Here to Help. Your premier destination for finding service providers. </Text>
          <Text className="text-gray-700">You have successfully registered to our platform using your {userDetails?.provider.charAt(0).toUpperCase()}{userDetails?.provider.slice(1)} account. </Text>
          <Text className="text-gray-700">You can now simply login to our website using your {userDetails?.provider.charAt(0).toUpperCase()}{userDetails?.provider.slice(1)} account and benefit from our services that are tailored to ease your service search process.</Text>
